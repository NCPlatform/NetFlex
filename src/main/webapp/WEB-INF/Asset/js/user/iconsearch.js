$(function(){
    ...
    $(document).click(function() {
    	if($('#searchBoxInput').val() != ""){
    		...
    		$.ajax({
            url: '/NetFlex/movie/search',
            type: 'GET',
            data: 'value=' + $('#searchBoxInput').val(),
            dataType: 'json',
            async: false,
            success: function (data) {
                ...
                $.each(data,function(index, items){
                	$('<div/>',{
                		...
                		id: 'openWatchAllJSP',
                		'data-seq-movie': items.seqMovie,
						'data-title': items.title,
						'data-story': items.story,
						'data-nation': items.nation,
						'data-director': items.director,
						'data-actor': items.actor,
						'data-year': items.year,
						'data-releasedate': items.releasedate,
						'data-liked': items.liked,
						'data-genrecode1': items.genrecode1,
						'data-genrecode2': items.genrecode2,
						'data-genrecode3': items.genrecode3,
						'data-movie-srcurl': items.movieSrcUrl,
						'data-movie-hit': items.hit,
						'data-thumbnail-src': items.thumbnailSrc,
						'data-thumbnail-srcurl': items.thumbnailSrcUrl,
						text : '재생'
                	})).append($('<button/>',{
                		class: 'btn btn-primary details-button2',
                		id: 'openModal',
                		'data-seq-movie': items.seqMovie,
						'data-title': items.title,
						'data-story': items.story,
...
$(document).on("click",".details-button2", function(){
		...
		$('#likeCount').text($(this).attr("data-liked"));
...
$(document).on("click","#openWatchJSP",function(){
    window.location.href = '/NetFlex/movie/watch';

});

$(document).on("click",'.details-button',function(){
	var movieSrcUrl = $(this).data("movie-srcurl");
    window.location.href = '/NetFlex/movie/watchAll?movieSrcUrl=' + encodeURIComponent(movieSrcUrl);
});
...
$('#logout').click(function(){
	$.ajax({
		...
		success: function(data){
			alert("로그아웃에 성공했습니다.")
			window.location.href="/NetFlex/account"
